      ...
      <div className="flex-1 flex flex-col min-w-0 p-4 lg:p-6 overflow-y-auto w-full relative z-0">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className={`hidden sm:flex px-6 py-3 rounded-xl font-bold transition-all shadow-md items-center gap-2 ${!!activeSession
        ...
        <div className="space-y-3">
          {getSessionHistory.isLoading && <p className="text-text-muted text-sm">Carregando sessões...</p>}
          {!getSessionHistory.isLoading && getSessionHistory.data?.data?.length === 0 && (
            <p className="text-text-muted text-sm text-center py-4 bg-background-surface rounded-xl border border-border-subtle">Nenhuma sessão recente. Inicie uma nova sessão!</p>
          )}
          {getSessionHistory.data?.data?.map((session: any) => (
            <div key={session.id} className="bg-background-surface border border-border-subtle rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-accent-primary/50 transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-background-elevated rounded-full flex items-center justify-center text-xl shrink-0">
                  {session.subject === 'Programação' ? '💻' : session.subject === 'Matemática' ? '📐' : session.subject === 'Biologia' ? '🧬' : '📚'}
                </div>
                <div>
                  <h4 className="font-bold text-text-primary text-sm sm:text-base">{session.subject}</h4>
                  <p className="text-xs sm:text-sm text-text-muted font-mono">{Math.floor((session.duration || 0) / 60)} Min • {session.mode === 'POMODORO' ? 'Pomodoro' : 'Avulso'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-4 bg-background-base px-3 py-1.5 rounded-lg border border-border-subtle shrink-0">
                <span className="text-sm font-bold text-accent-primary drop-shadow-sm">+{session.xpGained || 0} XP ⚡</span>
              </div>
            </div>
          ))}
        </div>
